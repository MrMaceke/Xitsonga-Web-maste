...
class SMSAppBotUtil {
    ...
    public function processQuery($queryString, $user) {
        if (SMSAppBotUtil::$isLive == false) {
            return SMSAppBotUtil::$offlineMessage;
        }
    ...
    public function processNumber($number) {
        ...
        $aJSON = "\"number\":\"$number\"," . "\"langauge\":\"Number\"," . "\"version\": \"SMS\"";
        $aJSON = json_decode("{" . $aJSON . "}");
    ...
    public function processClock($time) {
        $aWebBackend = new WebBackend();
        $aJSON = "\"time\":\"$time\"," . "\"langauge\":\"Time\"," . "\"version\": \"SMS\"";
    ...
    public function processSaveList($listName) {
        ...
        if (in_array($listName, SMSAppBotUtil::$aSaveListMenu) == FALSE) {
            $default = $default . "Bot supports saving following lists:";
            $default = $default . "\n";

            $return = $this->generateGenericMenu($default, SMSAppBotUtil::$aSaveListMenu);
        ...
        $aJSON = "\"sub_type\":\"$listName\"," . "\"documentType\":\"$format\"," . "\"version\":\"SMS\"";
        $aJSON = json_decode("{" . $aJSON . "}");
    ...
    public function processList($listName) {
        ...
        if (in_array($listName, SMSAppBotUtil::$aListMenu) == FALSE) {
            $default = $default . "Bot supports following lists:";
            $default = $default . "\n";

            $return = $this->generateGenericMenu($default, SMSAppBotUtil::$aListMenu);
    ...
    public function processHelp() {
        $default = $default . "I am Risuna\nA SMS Bot to help you learn Xitsonga.";
        $default = $default . "\n";

        return $this->generateGenericMenu($default, SMSAppBotUtil::$aFullMenuDescriptions);
    }
    ...
    public function routeRequest($queryString, $user) {
        ...
        $request = SMSRequest::toOrdinal($aRequestClassfication);
        return $request;
    }
    ...
    public function eligibleImage($queryString, $user) {
        ...
        $request = $this->routeRequest($queryString, $user);
        if ($request == SMSRequest::toOrdinal("t") || $request == SMSRequest::toOrdinal("w")) {
    ...
    public function generateFullMenu() {
        $default = $default . "SMS Bot to help you learn Xitsonga.";
        $default = $default . "\n";

        foreach (SMSAppBotUtil::$aFullMenu as $key => $value) {
            $default = $default . "\n$value";
        }
