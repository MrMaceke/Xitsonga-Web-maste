    ...
    class HTMLDisplay{
        ...
        public static function GetSingleRichTextEntityHTMLList($aResult) {
            ...
            if($aImage != "" AND $aVideo == ""){
                if(ucfirst($array[ItemTypeDAO::$ENGLISH_TRANS][content]) == "-" or $aType == ""){
                    $aDisplay = $aDisplay."<div class='desc_heading'><h4 style ='width:85%;float:left' id='$aResult[entity_id]'>".ucfirst($aResult[entity_name])."</h4></div>";
            ...
            $aRating = HTMLDisplay::getRatingPlugin($aResult[entity_id],$aRatingValue,0, $aCurrentRatingArray[0]);
        ...
        public static function GetSingleEntityHTMLList($aResult, $which = 0) {
            ...
            if($aImage != ""){
                ...
                if(ucfirst($array[ItemTypeDAO::$ENGLISH_TRANS][content]) == "-" or $aType == "proverbs"){
                    $aDisplay = $aDisplay."<div class='desc_heading'><h4 style ='width:85%;float:left' class ='red' id='$aResult[entity_id]'>".ucfirst($aResult[entity_name])."</h4></div>";
                    //$aDisplay = $aDisplay."<a style = 'float:right;padding:15px;font-size:12px' id ='openSendUserMailModalButton' href ='$_SERVER[REQUEST_URI]#'>< Edit ></a></div>";
                }else{
                   ...
                   $aSearch =  $array[ItemTypeDAO::$ENGLISH_TRANS][content];
                    ...
                    $aDisplay = $aDisplay."<div class='desc_heading'><h4 style ='width:85%;float:left' class ='red' id='$aResult[entity_id]'>".ucfirst($aResult[entity_name])." - ".ucfirst($aSearch)." $dictionaryType</h4></div>";
                    //$aDisplay = $aDisplay."<a style = 'float:right;padding:15px;font-size:12px' id ='openSendUserMailModalButton' href ='$_SERVER[REQUEST_URI]#'>< Edit ></a></div>";
		...
		$title = ucfirst($aResult['entity_name'])." is an English word meaning &quot;".ucfirst($array[ItemTypeDAO::$ENGLISH_TRANS][content]). "&quot; in Xitsonga. ";
            ...
            if($aType =="english" OR $aType =="xitsonga"){
                ...
                $aDisplay = $aDisplay."<span class =''><b>Oxford definition".strtolower("")."</b></span><hr>";
                
                $aFullDefinitions = DictionaryJSONCache::wordFullDefination($aSearch);
                if(count($aFullDefinitions) > 0) {
                    $aFullDefinitionsCount = 0;
                    foreach ($aFullDefinitions as $key => $Definitions) {
                        if($aFullDefinitionsCount == 0) {
                            $aDisplay = $aDisplay.ucfirst(trim($Definitions)).".<br/>";
                        } else {
                            $aDisplay = $aDisplay." - ". ucfirst(trim($Definitions)).".<br/>";
                        }
                        if($aFullDefinitionsCount > 1) {
                            break;
                        }
                        $aFullDefinitionsCount ++;
                    }
                    $aDisplay = $aDisplay."<hr/>Source: <a href='https://www.oed.com/'>https://www.oed.com</a>";
                } else {
                    $aDisplay = $aDisplay."No data found.<br/>";
                }
            ...
            if($aAuditResults[status] == true && false){
                ...
                $aAuditResults[message] = "Item has never been edited. ";
        ...
        public static function GetDisplayVideoListHTMLList($aResults, $aPageName, $aSubName) {
            ...
            foreach($aResults as $aResult){
                ...
                $url = HTMLDisplay::generateURL(strtolower(str_replace(" ","_",$aResult[entity_name])));
                $description = ucfirst($typeRecord[resultsArray][2]);
                $aDisplay = $aDisplay."<div><p><div style ='max-height:60px;overflow:hidden' class ='pull-left'><img src='assets/images/entity/$aImg' alt='' class='img-rounded pull-left' width='60' /></div><h5><a href ='$aPageName"."?_=$url'>".ucfirst($aResult[entity_name])."</a>"."</h5>$description</p><hr></div>";
            ...
            foreach($aResults as $aResult){
                ...
                if($aItemType == "proverbs" || $aItemType == "idioms"){
                    ...
                    if($explaination != ""){
						...
						$content = "<b>Direct - </b>".$array[ItemTypeDAO::$ENGLISH_TRANS][content];
					}
                } else {
                    $count = 100;
                    $content = HTMLDisplay::ucfirstSentence(str_replace(",",". ",$content,$count));
       ...
       public static function GetSayingDisplayHTMLList($aResults, $aPageName, $aSubName) {
		...
		$aDisplay = $aDisplay."<td width ='7%'>";
                ...
                $aDisplay = $aDisplay."<td>";
                $aDisplay = $aDisplay."<pre>".ucfirst(htmlspecialchars($aResult[translation]))."</pre>";
                $aDisplay = $aDisplay."</td>";
		...
		$aDisplay = $aDisplay."<td width ='7%'>";
                ...
                $aDisplay = $aDisplay."<td class = 'item'>";
                $aDisplay = $aDisplay.($aResult[item]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'replacement'>";
                $aDisplay = $aDisplay.($aResult[replacement]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'pattern'>";
                $aDisplay = $aDisplay.($aResult[pattern]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'language'>";
                $aDisplay = $aDisplay.($aResult[language]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'swap_left'>";
                $aDisplay = $aDisplay.($aResult[swap_left]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'swap_right'>";
                $aDisplay = $aDisplay.($aResult[swap_right]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'push_first'>";
                $aDisplay = $aDisplay.($aResult[push_first]);
                $aDisplay = $aDisplay."</td>";
				
		$aDisplay = $aDisplay."<td class = 'push_last'>";
                $aDisplay = $aDisplay.$aResult[push_last];
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td>";
                $aDisplay = $aDisplay."<a class = 'edit_config' id = '$aResult[config_id]' href='#'>Edit</a>";
                $aDisplay = $aDisplay."</td>";
        ...
        public static function GetTranslationTrainingHTMLList($aResults) {
            $aDisplay = "<table id ='audits_api_data_table' class='display' cellspacing='0' width='100%'>";
            $aDisplay = $aDisplay
                    ."<thead>"
                    ."<tr>"
		...
		$aDisplay = $aDisplay."<td width ='7%'>";
                ...
                $aDisplay = $aDisplay."<td class = 'item'>";
                $aDisplay = $aDisplay.($aResult[input]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'replacement'>";
                $aDisplay = $aDisplay.($aResult[output]);
                $aDisplay = $aDisplay."</td>";
                ...
                $aDisplay = $aDisplay."<td class = 'language'>";
                $aDisplay = $aDisplay.($aResult[build]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td class = 'swap_left'>";
                $aDisplay = $aDisplay.($aResult[rating]);
                $aDisplay = $aDisplay."</td>";
                
                $aDisplay = $aDisplay."<td>";
                //$aDisplay = $aDisplay."<a class = 'edit_config' id = '$aResult[config_id]'>Edit</a>";
                $aDisplay = $aDisplay."</td>";
        ...
        public static function getRatingPlugin($aID,$aRatingValue, $rating = NULL, $people = NULL){
            ...
            //$aReturn = $aReturn."<span class ='rating_message'>$aRatingMesssage</span><span class ='rating_message'>Help improve content quality by rating below.</span>";
