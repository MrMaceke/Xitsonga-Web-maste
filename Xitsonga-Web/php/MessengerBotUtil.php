...
class MessengerBotUtil {
    ...
    public static $aFullMenuDescriptions = array(
        ...
        "/L gives list\nLists a category of words\nExample \"List months\" or \"L months\"\n",
    ...
    public function processQuery($queryString, $user) {
        if (MessengerBotUtil::$isLive == false) {
            return MessengerBotUtil::$offlineMessage;
        }
        ...
        switch ($request) {
            case MessengerRequest::toOrdinal("t"):
                $return = $this->processTranslatePhrase($body);
                break;
            case MessengerRequest::toOrdinal("w"):
                $return = $this->processTranslateWord($body);
                if (trim($return) == "") {
                    $return = $this->processTranslatePhrase($body);
                }
                break;
            case MessengerRequest::toOrdinal("l"):
                $return = $this->processList($body);
                break;
            case MessengerRequest::toOrdinal("p"):
                $return = $this->processSaveList($body);
                break;
            //case MessengerRequest::toOrdinal("a"):
            //$return = "Sorry, I currently don't support audio.";
            //break;
            case MessengerRequest::toOrdinal("c"):
                $return = $this->processClock($body);
                break;
            case MessengerRequest::toOrdinal("n"):
                $return = $this->processNumber($body);
                break;
            case MessengerRequest::toOrdinal("h"):
                $return = $this->processHelp();
                break;
            default:
        ...
        if (trim($return) == "") {
            $return = "Sorry, I am struggling to translate \"$queryString\".";
        }
        return $return;
    }
    ...
    public function processNumber($number) {
        ...
        $aJSON = "\"number\":\"$number\"," . "\"langauge\":\"Number\"," . "\"version\": \"Messenger\"";
        $aJSON = json_decode("{" . $aJSON . "}");
    ...
    public function processClock($time) {
        $aWebBackend = new WebBackend();
        $aJSON = "\"time\":\"$time\"," . "\"langauge\":\"Time\"," . "\"version\": \"Messenger\"";
    ...
    public function processSaveList($listName) {
        ...
        if (in_array($listName, MessengerBotUtil::$aSaveListMenu) == FALSE) {
            $default = $default . "Bot supports saving following lists:";
            $default = $default . "\n";

            $return = $this->generateGenericMenu($default, MessengerBotUtil::$aSaveListMenu);
        ...
        $aJSON = "\"sub_type\":\"$listName\"," . "\"documentType\":\"$format\"," . "\"version\":\"Messenger\"";
        $aJSON = json_decode("{" . $aJSON . "}");
    ...
    public function processList($listName) {
        ...
        if (in_array($listName, MessengerBotUtil::$aListMenu) == FALSE) {
            $default = $default . "Bot supports following lists:";
            $default = $default . "\n";

            $return = $this->generateGenericMenu($default, MessengerBotUtil::$aListMenu);
    ...
    public function processHelp() {
        $default = $default . "I am Risuna\nA FB Messenger Bot to help you learn Xitsonga.";
        $default = $default . "\n";

        return $this->generateGenericMenu($default, MessengerBotUtil::$aFullMenuDescriptions);
    }
    ...
    public function routeRequest($queryString, $user) {
        ...
        $request = MessengerRequest::toOrdinal($aRequestClassfication);
        return $request;
    }
    ...
    public function eligibleImage($queryString, $user) {
        ...
        $request = $this->routeRequest($queryString, $user);
        if ($request == MessengerRequest::toOrdinal("t") || $request == MessengerRequest::toOrdinal("w")) {
    ...
    public function generateFullMenu() {
        $default = $default . "FB Messenger Bot to help you learn Xitsonga.";
        $default = $default . "\n";

        foreach (MessengerBotUtil::$aFullMenu as $key => $value) {
            $default = $default . "\n$value";
        }
