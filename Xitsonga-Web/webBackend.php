...
require_once './php/TranslatoXitsongaUtil.php';
require_once './php/TranslationConfigDAO.php';
require_once "./php/WhatsAppBotUtil.php";
require_once "./php/MessengerBotUtil.php";
require_once "./php/SMSAppBotUtil.php";
...
if (isset($_REQUEST['system']) && ($_REQUEST['system'] == "android" || $_REQUEST['system'] == "iOS")) {
    ...
    } else if ($aMethod == "WhatsAppBot") {
        $data = trim(file_get_contents('php://input'));
        $queryString = $_REQUEST[Body];
        $user = $_REQUEST[From];
        $mediaURL = $_REQUEST[MediaUrl0];

        echo $aWebBackend->processWhatsAppRequestLive($queryString, $mediaURL, $user, "");
    }else if ($aMethod == "CallAppBot") {
        $data = trim(file_get_contents('php://input'));

        echo $aWebBackend->processCallRequestLive();
    } else if ($aMethod == "WhatsAppBotOffline") {
        return;
    } else if ($aMethod == "SMSAppBotLive") {
        $data = trim(file_get_contents('php://input'));
        $queryString = $_REQUEST[Body];
        $user = $_REQUEST[From];

        echo $aWebBackend->processSMSRequestLive($queryString, $user, "");
    } else if ($aMethod == "FBMessengerBotLive") {
        $data = trim(file_get_contents('php://input'));

        if (JSONDisplay::IsJson($data) == false) {
            $data = $data . "\"}";
        }

        //file_put_contents("open/json", $data);

        if (JSONDisplay::IsJson($data)) {
            $data = json_decode($data);

            $queryString = $data->text;
            $user = $data->userId;
            $messageId = $data->messageId;

            echo $aWebBackend->processMessengerRequestLive($queryString, $user, $messageId);
        } else {
            echo $aWebBackend->processMessengerRequestLive("Empty", "System User", 4);
        }
    } else if ($aMethod == "FBMessengerBotMediaLive") {
        $data = trim(file_get_contents('php://input'));
        if (JSONDisplay::IsJson($data)) {
            $data = json_decode($data);
            ...
            $type = $data->mediaType;
            $user = $data->userId;
            $messengerId = $data->messageId;

            echo $aWebBackend->processMessengerMediaStoreRequestLive($url, $user, $messengerId);
        } else {
            echo $aWebBackend->processMessengerMediaStoreRequestLive($url, "System User", 0);
        }
    } else if ($aMethod == "processVision") {

        echo $aWebBackend->processVision();
    } else if ($aMethod == "processVision") {

        echo $aWebBackend->processVision();
    } else if ($aMethod == "liveTranslator") {
        $text = strtolower($_REQUEST['text']);
        $language = strtolower($_REQUEST['language']);
        $caller = strtolower($_REQUEST['caller']);
        $url = strtolower($_REQUEST['url']);
        $firstTime = strtolower($_REQUEST['firstTranslation']);

        $aJSON = "\"text\":\"$text\"," . "\"langauge\":\"$language\"," . "\"version\":\"$caller\"," . "\"url\":\"$url\"," . "\"firstTime\":\"$firstTime\"";

        $aJSON = json_decode("{" . $aJSON . "}");

        echo $aWebBackend->liveTranslate($aJSON);
    } else if ($aMethod == "generateDictionaryJSON") {

        $aWebBackend->generateDictionaryJSON();

        echo "Refreshing dictionary cache completed";
    } else if ($aMethod == "getFileContent") {
        $fileName = $_REQUEST[file];

        $content = file_get_contents("./php/translator_categories/" . $fileName);

        $content = preg_replace("/\r\n|\r|\n/", '<br/>', $content);

        echo $content;
    } else if ($aMethod == "sendSystemUpdate") {

        $aWebBackend->sendSystemUpdate();
    } else if ($aMethod == "generateTranslatorConfigJSON") {

        $aWebBackend->generateTranslatorConfigsJSON("english");
        $aWebBackend->generateTranslatorConfigsJSON("english_spelling");
        $aWebBackend->generateTranslatorConfigsJSON("xitsonga");
        $aWebBackend->generateTranslatorConfigsJSON("xitsonga_spelling");

        echo "Refreshing translations configurations completed";
...
class WebBackend {
    ...
    public static $BOT_LIMIT_MESSAGE = "It was nice chatting to you today. I am happy to help you learn and translate again tomorrow.";
    public static $BOT_USAGE_LIMIT = 100;
    ...
    public function translation($data) {
        ...
        $aData[translation] = strtoupper($aFormat);
        $aData[caller] = "Web";
        $aData[type] = "API";
        ...
        if ($aValidation[status]) {
            ...
            if (!in_array(strtolower($param->sub_type), $array)) {
                ...
                if ($param->sub_type == "xitsonga" or $param->sub_type == "english") {
                    $title = "List " . $param->sub_type . " words translated from Xitsonga to English";
                } else {
                    $title = "List of " . $param->sub_type . " translated from Xitsonga to English";
                }
            }
            ...
            if ($aResults[status]) {
                ...
                if ($param->documentType == "text") {
                    ...
                    $aData[item] = $param->sub_type;
                    $aData[translation] = "TXT";
                    $aData[type] = "Document";
                    $aData[caller] = $param->version == "" ? "web" : $param->version;
                ...
                } else if ($param->documentType == "csv") {
                    $aContents = "Item;Translation\n";
                    if ($param->sub_type == "proverbs" or $param->sub_type == "idioms") {
                        $aContents = "Item;Direct;Meaning\n";
                    }
                    $aCount = 1;
                    foreach ($aResults[resultsArray] as $key => $value) {
                        ...
                        if ($param->sub_type == "homonyms" or $param->sub_type == "antonyms") {
                            $aContents = $aContents . str_replace(";", ". ", ucfirst($value[entity_name])) . " - " . str_replace(";", ". ", $aTranslation) . " ; " . str_replace(";", ". ", ucfirst($array[ucfirst($param->sub_type)][content])) . "\n";
                        } else if ($param->sub_type == "proverbs" or $param->sub_type == "idioms") {
                            $aContents = $aContents . str_replace(";", ". ", ucfirst($value[entity_name])) . ";" . str_replace(";", ". ", $aTranslation) . ";" . str_replace(";", ". ", ucfirst($aExplaination)) . "\n";
                        } else {
                            $aContents = $aContents . str_replace(";", ". ", ucfirst($value[entity_name])) . ";" . str_replace(";", ". ", $aTranslation) . "\n";
                        }

                        $aCount ++;
                    }

                    $file = "generated/$aFileName.csv";
                    ...
                    return $this->formatSuccessFeebackToJSON($aFileName . ".csv");
                } if ($param->documentType == "text") {
                    $aContents = "Xitsonga.org\n\n$title\n\n";
                    $aCount = 1;
                    foreach ($aResults[resultsArray] as $key => $value) {
                    ...
                    return $this->formatSuccessFeebackToJSON($aFileName . ".txt");
                } else if ($param->documentType == "tsv") {
                    $aCount = 1;
                    foreach ($aResults[resultsArray] as $key => $value) {
                    ...
                    return $this->formatSuccessFeebackToJSON($aFileName . ".tsv");
                } else {
                    $aCount = 1;
    ...
    public function getTranslationForWordNew() {
        ...
        $aResults = $aDefinationCacheDAO->findCacheByName($aSearch, "mobile");
        if ($aResults[status]) {
            return $aResults[resultsArray][0][defination];
        }
    ...
    public function addTranslationConfig($data) {
        ...
        if (in_array($firstWord, $firstPerson)) {
            foreach ($firstPerson as $key => $value) {
                
            }
            return $this->formatSuccessFeebackToJSON("First person translation added successfully", OPERATION_SUCCESS);
        }

        return $this->processAddTranslationConfig($data);
    }

    public function processAddTranslationConfig($data) {
        $aInputValidator = new InputValidator();
     ...
     * @return JSON
     */
    public function editTranslationConfig($data) {
        //Validate input
        $aInputValidator = new InputValidator();
        $aValidation = $aInputValidator->inputValidationEditTranslationConfig($data);
        if (!$aValidation[status]) {
            return $this->formatErrorFeebackToJSON($aValidation['message']);
        }
        ...
        $aResult = $aTranslationConfigDAO->updateTranslationConfig($data, $this->getCurrentUser()->getUserID());

        if ($aResult['status']) {
            return $this->formatSuccessFeebackToJSON("Translation updated successfully", OPERATION_SUCCESS);
        } else {
            return $this->formatErrorFeebackToJSON($aResult['message']);
        }
    }
     ...
     * @return JSON
     */
    public function removeTranslationConfig($data) {
        //Validate input
        $aInputValidator = new InputValidator();
        $aValidation = $aInputValidator->inputValidationRemoveTranslationConfig($data);
        if (!$aValidation[status]) {
            return $this->formatErrorFeebackToJSON($aValidation['message']);
        }
        ...
        $aResult = $aTranslationConfigDAO->deleteTranslationConfigDetails($data->config_id);
        $aResult = $aTranslationConfigDAO->deleteTranslationConfig($data->config_id);
        if ($aResult['status']) {
            return $this->formatSuccessFeebackToJSON("Translation removed successfully", OPERATION_SUCCESS);
        } else {
            return $this->formatErrorFeebackToJSON($aResult['message']);
        }
    }
    ...
    public function addEntityImageDetail($aImageName, $entityId, $aUserID) {
        ...
        return $aEntityDetails->addEntityImageDetail($aImageName, $entityId, $aUserID);
    } 
    ...
    public function registerUser($data) {
        ...
        if (true && strtolower($data->email) != "hlamarisonelly@gmail.com") {
            return $this->formatErrorFeebackToJSON("We are currently updating the website. Registration is suspended until 1 April 2020");
        }
     ...
     * @return HTML content
     */
    public function allowResetPassword($aActivationCode) {
        $aActivationDAO = new ActivationDAO();
        $aUserDAO = new UserDAO();

        $aResult = $aActivationDAO->getActivationByHash($aActivationCode);
        if ($aResult['status']) {
            $aPendingPasswordReset = $aResult[resultsArray][password_reset];
            if ($aPendingPasswordReset == "0") {
                return $aPendingPasswordReset;
            } else {
                return "Password code has expired.";
            }
        }
        return "Password code does not exist.";
    }
     ...
     * @return HTML content
     */
    public function retrieveResetPasswordEmail($aActivationCode) {
        $aActivationDAO = new ActivationDAO();
        $aUserDAO = new UserDAO();
    ...
    public function askTimeAppNew($data) {
        ...
        $aTime = trim($data->time);
        if (strlen($aTime) < 5) {

            if (strlen($aTime) == 0) {
                $t = time();
                $aTime = date('h:i');
            } else if (strlen($aTime) == 1) {
                $aTime = "0" . $aTime . ":00";
            } else if (strlen($aTime) == 2) {
                $aTime = $aTime . ":00";
            } else if (strlen($aTime) == 3) {
                $aTime = $aTime . "00";
            } else if (strlen($aTime) == 4) {
                $temp1 = substr($aTime, 0, 2);
                $temp2 = substr($aTime, 2, 4);
                $aTime = $temp1 . ":" . $temp2;
            }
        }
        $aDate = $aDate . " " . trim($aTime);

        $aTsongaTimer = new TsongaTime();
        if (preg_match("/^(?:2[0-3]|[01][0-9]):[0-5][0-9]$/", trim($aTime))) {
        ...
        return $this->formatSuccessFeebackToJSON($aTime . " - " . $time);
    }
    ...
    public function askTime($data) {
        if (!preg_match("/^(?:2[0-3]|[01][0-9]):[0-5][0-9]$/", trim($data->time))) {
    ...
    public function processAzureVisionAI($image, $aMessageSid) {
        $url = 'https://southafricanorth.api.cognitive.microsoft.com/vision/v2.0/describe?maxCandidates=1&language=en';
        $data = array(
            "url" => $image
        );
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json', "Ocp-Apim-Subscription-Key: 20582cea36fb4e0c9e32873e90b004e3"));
        $result = curl_exec($ch);

        curl_close($ch);

        file_put_contents("azure/" . $aMessageSid . ".json", $result);

        $words = array();
        $json = json_decode($result);

        $count = 0;
        for ($index = 0; $index < count($json->description->captions); $index ++) {
            $word = strtolower($json->description->captions[$index]->text);
        ...
        $count = 0;
        for ($index = 0; $index < count($json->description->tags); $index ++) {
            $word = strtolower($json->description->tags[$index]);
            if (in_array($word, $words) == false) {
                array_push($words, $word);
                $count ++;
            }
    ...
    public function processGoogleSpeechToText($audio, $aMessageSid) {
        $path = $audio;
        $type = pathinfo($path, PATHINFO_EXTENSION);
        $data = file_get_contents($path);
        $base64 = base64_encode($data);

        $url = 'https://speech.googleapis.com/v1p1beta1/speech:recognize?key=AIzaSyBClQoCLS44dDW4y7RQiHqREXSPQIwxFn8';
        $data = array(
            "config" => array(
                "encoding" => "MP3",
        ...
        file_put_contents("google/speech-to-text/" . $aMessageSid . ".json", $result);

        $json = json_decode($result);

        for ($index = 0; $index < count($json->results[0]->alternatives); $index ++) {
            $transcript = strtolower($json->results[0]->alternatives[$index]->transcript);
            break;
        }
        return $transcript;
    }

    public function processGoogleVisionAI($image, $aMessageSid) {
        $path = $image;
        $type = pathinfo($path, PATHINFO_EXTENSION);
        $data = file_get_contents($path);
        $base64 = base64_encode($data);

        $url = 'https://vision.googleapis.com/v1/images:annotate?key=AIzaSyBClQoCLS44dDW4y7RQiHqREXSPQIwxFn8';
        $data = array(
            "requests" => array(
                "image" => array(
                    "content" => $base64
                //"source" => array("imageUri" => "$image")
                ),
                "features" => array(
                    array(
        ...
        // Save response
        file_put_contents("google/vision/" . $aMessageSid . ".json", $result); 
        
        $words = array();
        $json = json_decode($result);
        $count = 0;
        for ($index = 0; $index < count($json->responses[0]->labelAnnotations); $index ++) {
            $word = strtolower($json->responses[0]->labelAnnotations[$index]->description);
            if (in_array($word, $words) == false) {
                array_push($words, $word);
                $count ++;
            }
        ...
        /*
          $count = 0;
          for ($index = 0; $index < count($json->responses[0]->webDetection->webEntities); $index ++) {
          $word = strtolower($json->responses[0]->webDetection->webEntities[$index]->description);
          if(in_array($word, $words) == false) {
          array_push($words, $word);
          $count ++;
          }
     ...
     * @return type
     */
    public function processCallRequestLive() {
        $aXMLUtil = new XMLUtils();
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        
        $user = $_REQUEST[From];
        
        $aData[translation] = "Phone call";
        $aData[item] = $user;
        $aData[type] = "PhoneCallBot";
        $aData[caller] = "PhoneCall";

        $aAuditsAPICallsDAO->AddAuditAPITrail($aData);
        
        return $aXMLUtil->trilioCallResponse("http://xitsonga.org/audio/greetingOne.mp3");
    }
     ...
     * @return type
     */
    public function processWhatsAppRequestLive($queryString, $mediaURL, $user, $data) {
        $aWhatsAppBotUtil = new WhatsAppBotUtil();
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $aXMLUtil = new XMLUtils();

        $aAlreadyProcess = $aAuditsAPICallsDAO->retrieveBotMessageWithId($_REQUEST[MessageSid]);
        if ($aAlreadyProcess[status]) {
            $aAuditsAPICallsDAO->increaseRetryCountBotMessageWithId($_REQUEST[MessageSid]);
            return;
        }
        ...
        $aUsageCount = $aAuditsAPICallsDAO->retrieveUserBotMessageByDate($user, date("Y-m-d") . " 00:00:00");
        if ($aUsageCount[status] && $aUsageCount[resultsArray][0][count] > WebBackend::$BOT_USAGE_LIMIT) {
            $aMessage = WebBackend::$BOT_LIMIT_MESSAGE;
            $aAuditsAPICallsDAO->AddBotMessengerTrail($user, $_REQUEST[MessageSid]);

            if ($aUsageCount[resultsArray][0][count] > WebBackend::$BOT_USAGE_LIMIT + 1) {
                return;
            }

            return $aXMLUtil->trilioSMSResponse($aMessage);
        }

        $aAuditsAPICallsDAO->AddBotMessengerTrail($user, $_REQUEST[MessageSid]);

        $mediaType = $_REQUEST["MediaContentType0"];
        if (TranslatorUtil::has_prefix($mediaType, "audio")) {
            $extention = explode("/", $mediaType)[1];
            $audio = $this->saveAudioLocally($mediaURL, $_REQUEST[MessageSid], $extention);
            if ($audio == NULL) {
                $aMessage = "I am only able to understand text, images and audio.";
                $aData[translation] = $aMessage;
            } else {
                $queryString = "Speech";

                $text = $this->processGoogleSpeechToText($audio, $_REQUEST[MessageSid]);
                if (strlen($text) == 0) {
                    $aMessage = "I am struggling to understand your audio.";
                    $aData[translation] = $aMessage;
                }

                $aMessage = $aWhatsAppBotUtil->processQuery("t $text", $user);
                if ($aMessage == TranslatorService::$emptyValuePassed) {
                    $aMessage = "I am struggling to understand your audio.";
                }
            }

            WebBackend::audit("Speech-to-text", $text, $aMessage, "english", "WhatsApp");
        } else if (TranslatorUtil::has_prefix($mediaType, "image")) {
            $url = $this->saveImageLocally($mediaURL, $_REQUEST[MessageSid]);
            if ($url == NULL) {
                $aMessage = "I am only able to understand text and images.";
                $aData[translation] = $aMessage;
            } else {
                $queryString = "Vision";

                $visionWords = $this->processGoogleVisionAI($url, $_REQUEST[MessageSid]);
                if (count($visionWords) == 0) {
                    $visionWords = $this->processAzureVisionAI($url, $_REQUEST[MessageSid]);
                }
                
                SanitizeUtil::deleteFile($url); 

                if (count($visionWords) == 0) {
                    $aMessage = "I am struggling to understand your image.";
                    $aData[translation] = $aMessage;
                } else {
                    $returnMessage = "";
                    foreach ($visionWords as $key => $visionWord) {
                        $aMessage = $aWhatsAppBotUtil->processQuery("t $visionWord", $user);

                        if (strtolower(trim($aMessage)) != strtolower(trim($visionWord))) {
                            $aMessage = SanitizeUtil::cleanVisionAIResponse($visionWords, $aMessage);

                            $returnMessage = $returnMessage . $aMessage . ". ";
                        }
                    }
                    $returnMessage = trim($returnMessage);

                    $visionWordsString = "";
                    foreach ($visionWords as $key => $value) {
                        $visionWordsString = $visionWordsString . $value . ". ";
                    }
                    $visionWordsString = trim($visionWordsString);
                    $aData[translation] = "Vision" . " -  " . $visionWordsString;

                    $aMessage = $returnMessage;
                }

                WebBackend::audit("Vision", $visionWordsString, $aMessage, "english", "WhatsApp");
            }
        } else {
            $aMessage = $aWhatsAppBotUtil->processQuery($queryString, $user);
            $imageURL = $aWhatsAppBotUtil->eligibleImage($queryString, $user);
            /**
             * Audits requests
             */
            if (strpos($aMessage, 'https://') !== false) {
                $imageURL = $aMessage;
                $aData[translation] = "Document" . " -  " . $imageURL;
            } else if ($imageURL == "") {
                $aData[translation] = $aMessage;
            } else {
                $aData[translation] = $aMessage . " - (With Image).";
            }
        }

        $aData[item] = $queryString . " - " . $user;
        $aData[type] = "WhatsAppBot";
        $aData[caller] = "WhatsApp";

        $aAuditsAPICallsDAO->AddAuditAPITrail($aData);

        if ($imageURL != "" || strpos($aMessage, 'https://') !== false) {
            return $aXMLUtil->trilioMediaSMSResponse($aMessage, $imageURL);
        }
        return $aXMLUtil->trilioSMSResponse($aMessage);
    }

    public function getImageBaseString($url, $messageId) {
        $image = file_get_contents('https://media.geeksforgeeks.org/wp-content/uploads/geeksforgeeks-22.png');

        return base64_encode($image);
    }

    public function saveImageLocally($url, $messagerId) {
        $savePath = "aws/" . $messagerId . ".jpg";
        ...
        return "https://www.xitsonga.org/$savePath";
    }

    public function saveAudioLocally($url, $messagerId, $extention) {
        $savePath = "speech-audio/" . $messagerId . ".$extention";
     ...
     * @return type
     */
    public function processSMSRequestLive($queryString, $user, $data) {
        $aSMSAppBotUtil = new SMSAppBotUtil();
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $aXMLUtil = new XMLUtils();
        ...
        $aMessage = $aSMSAppBotUtil->processQuery($queryString, $user);
    ...
    public function processVision() {
        $image = $_FILES['image']['name'];
        $extImage = explode('.', $image);
        $imageType = $extImage[1];
        ...
        $bannerpath = "aws/" . $imageName . '.' . $imageType;
        ...
        $callerMessage = "";
        foreach ($visionWords as $key => $visionWord) {
            $aJSON = json_decode("{\"text\":\"$visionWord\"," . "\"langauge\":\"english\"," . "\"version\":\"Vision\"}");
            $response = json_decode($this->liveTranslate($aJSON));

            $aMessage = trim($response->infoMessage);
        ...
        $aData[translation] = $aMessage;
        $aData[item] = $callerMessage;
        $aData[type] = "Vision";
        $aData[caller] = "Web";

        $aAuditsAPICallsDAO->AddAuditAPITrail($aData);

        return $this->formatSuccessFeebackToJSON("https://www.xitsonga.org/" . $bannerpath . "**" . $aMessage);
    }

    public function processVisionJSON($data) {
        $imageType = $extImage[1];
        ...
        $aData[item] = $callerMessage;
        $aData[type] = "Vision";
        $aData[caller] = $data->version == "" ? "Mobile" : $data->version;

        $aAuditsAPICallsDAO->AddAuditAPITrail($aData);

        return $this->formatSuccessFeebackToJSON($aMessage);
    }
     ...
     * @return type
     */
    public function processMessengerMediaStoreRequestLive($url, $user, $messengerId) {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();

        $aAlreadyProcess = $aAuditsAPICallsDAO->retrieveBotMessageWithId($messengerId);
        if ($aAlreadyProcess[status]) {
            $aAuditsAPICallsDAO->increaseRetryCountBotMessageWithId($messengerId);
            return $this->formatSuccessFeebackToJSON(JSONDisplay::formSafeJSON(("1")));
        }

        $aUsageCount = $aAuditsAPICallsDAO->retrieveUserBotMessageByDate($user, date("Y-m-d") . " 00:00:00");
        if ($aUsageCount[status] && $aUsageCount[resultsArray][0][count] > WebBackend::$BOT_USAGE_LIMIT) {
            $aMessage = WebBackend::$BOT_LIMIT_MESSAGE;
            $aAuditsAPICallsDAO->AddBotMessengerTrail($user, $messengerId);

            if ($aUsageCount[resultsArray][0][count] > WebBackend::$BOT_USAGE_LIMIT + 1) {
                return;
            }

            return $this->formatSuccessFeebackToJSON(JSONDisplay::formSafeJSON(($aMessage)));
        }

        $aAuditsAPICallsDAO->AddBotMessengerTrail($user, $messengerId);

        $image = $this->saveImageLocally($url, $messengerId);
        if ($image == null) {
            return $this->formatSuccessFeebackToJSON("I am only able to understand text and images.");
        }

        $visionWords = $this->processGoogleVisionAI($image, $messengerId);
        if (count($visionWords) == 0) {
            $visionWords = $this->processAzureVisionAI($url, $messengerId);
        }
        ...
        if (count($visionWords) == 0) {
            $aMessage = "I am not able to understand your image.";
            
            WebBackend::audit("Vision", $visionWordsString, $aMessage, "english", "Messenger");
            return $this->formatSuccessFeebackToJSON($aMessage);
        }

        $returnMessage = "";
        foreach ($visionWords as $key => $visionWord) {
            $response = json_decode($this->processMessengerRequestLive("t " . $visionWord, $user . " - AI", $messengerId . "_" . $visionWord, true));
     ...
     * @return type
     */
    public function processMessengerRequestLive($queryString, $user, $messangerId, $ignoreImage = false) {
        $aMessengerUtil = new MessengerBotUtil();
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();

        $aAlreadyProcess = $aAuditsAPICallsDAO->retrieveBotMessageWithId($messangerId);
        if ($aAlreadyProcess[status]) {
            $aAuditsAPICallsDAO->increaseRetryCountBotMessageWithId($messangerId);
            return $this->formatSuccessFeebackToJSON(JSONDisplay::formSafeJSON(("1")));
        }
        ...
        if ($aUsageCount[status] && $aUsageCount[resultsArray][0][count] > WebBackend::$BOT_USAGE_LIMIT) {
            $aMessage = WebBackend::$BOT_LIMIT_MESSAGE;
            $aAuditsAPICallsDAO->AddBotMessengerTrail($user, $messangerId);
        ...
        $aMessage = $aMessengerUtil->processQuery($queryString, $user);
        if ($ignoreImage == false) {
            $imageURL = $aMessengerUtil->eligibleImage($queryString, $user);
        }
        ...
        $aData[item] = $queryString . " - " . $user;
        $aData[type] = "MessengerBot";
        $aData[caller] = "Messenger";

        $aAuditsAPICallsDAO->AddAuditAPITrail($aData);

        if ($imageURL != "" || strpos($aMessage, 'https://') !== false) {
            $aMessage = $aMessage . "**" . $imageURL;
            return $this->formatSuccessFeebackToJSON(JSONDisplay::formSafeJSON(($aMessage)));
        }
    ...
    public function liveTranslate($data) {
        $adminUser = $this->getCurrentUser()->getEmail() == "sneidon@yahoo.com";

        return TranslatorService::translateText($data, $adminUser);
    }
    ...
    public function sendSystemUpdate() {

        $systems = array("web", "messenger", "sms", "whatsapp", "android", "ios");

        foreach ($systems as $key => $value) {

            if ($value == "android" && $value == "ios") {
                $data = $this->updateForFunction($data, "Word Descriptions", "xitsonga", "english", $value);
            }

            $data = $this->updateForFunction($data, "Translator", "Translate", "Translate", $value);
            $data = $this->updateForFunction($data, "Numbers & Time", "Number", "Time", $value);
        }

        $aSendMail = new SendMail();

        $aSendMail->sendSystemUpdateEmail($data);

        echo "Email sent";
    }

    public function updateForFunction($data, $name, $item1, $item2, $caller) {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $startDate = date("Y-m-d H:i:s", strtotime('-24 hours', time()));
        $endDate = date("Y-m-d H:i:s", strtotime('-48 hours', time()));

        $count = 10000;

        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByTypeAndSystem(strtolower($item1), strtolower($item2), $caller, $count, $startDate);
        $aYestedayResults = $aAuditsAPICallsDAO->listAuditAPICallsByTypeAndDateAndSystem(strtolower($item1), strtolower($item2), $caller, $count, $endDate, $startDate);

        $data["$caller" . "_$item1"][name] = $name;
        $data["$caller" . "_$item1"][visits] = count($aResults[resultsArray]);

        $today = count($aResults[resultsArray]);
        $yesterday = count($aYestedayResults[resultsArray]);

        $percentages = number_format($today / $yesterday * 100, 2);

        if ($today == 0 && $yesterday == 0) {
            $percentages = 0;
        } else if ($percentages > 0) {
            $percentages = $percentages - 100;
        } else if ($percentages == 0 && $today < $yesterday) {
            $percentages = -100;
        } else if ($percentages == 0 && $today > $yesterday) {
            $percentages = 100;
        }
        $percentages = round($percentages, 2);
        $data["$caller" . "_$item1"][yesterday] = $yesterday;
        $data["$caller" . "_$item1"][description] = "$percentages" . "%";
    ...
    public function generateDictionaryJSONNew() {
        $aEntityDAO = new EntityDAO();
        $aResults = $aEntityDAO->listEntityByTypeExport($data);
    ...
    public function generateTranslatorConfigsJSON($language) {
        $aTranslationConfigDAO = new TranslationConfigDAO();
        $aResults = $aTranslationConfigDAO->getTranslationConfigs($language);

        $content = JSONDisplay::GetTranslationConfigsJSON($aResults['resultsArray'], 999, "Configs retrieved successfully");
        if (!JSONDisplay::IsJson($content)) {
            return $this->formatErrorFeebackToJSON("Data is corrupt. Please investigate");
        }
        $file = "php/translator_categories/translator_configs_$language.json";
        $fh = fopen($file, 'w');
        fclose($fh);
    ...
    public function listEntityByType($data, $output = "html") {
        ...
        if ($aResults['status'] OR $data["page"] == "manage") {
            if ($output == "GENERATE_FILE") {
                ...
                } elseif ($data["sk"] == "jokes" or $data["sk"] == "poems" or $data["sk"] == "song-lyrics" or $data["sk"] == "traditional-food" or $data[sk] == "short-stories") {
    ...
    public function updateFile($param) {
        $filename = $param->fileName;
        $content = $param->content;

        $tags = array('</p>', '</div>', '<br />', '<br>', '<hr />', '<hr>', '</h1>', '</h2>', '</h3>', '</h4>', '</h5>', '</h6>');
        $content = str_replace($tags, "\n", $content);
        $content = strip_tags($content);

        file_put_contents("./php/translator_categories/" . $filename, $content);

        return $this->formatSuccessFeebackToJSON("File updated");
    }
    ...
    public function getEntityByURL($data, $output = "html") {
        ...
        if ($aResults['status']) {
            ...
            } elseif ($data[sk] == "jokes" or $data[sk] == "poems" or $data[sk] == "short-stories") {
    ...
    public function GetAuditAPICallsHTMLList() {
        ...
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType("xitsonga", "english", 500, $date);
        if ($aResults['status']) {
    ...
    public function GetAuditNumbersAPICallsHTMLList() {
        ...
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType("number", "Number", 500, $date);
        if ($aResults['status']) {
            return HTMLDisplay::GetAuditAPICallsHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
     ...
     * @return type
     */
    public function GetAuditGeneralAPICallsHTMLList($caller1, $caller2, $time, $limit) {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $date = date("Y-m-d H:i:s", strtotime($time, time()));
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType($caller1, $caller1, $limit, $date);
        if ($aResults['status']) {
            return HTMLDisplay::GetAuditAPICallsHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
    ...
    public function GetAuditTranslateAPICallsHTMLList() {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $date = date("Y-m-d H:i:s", strtotime('-4 weeks', time()));
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType("translate", "Translate", 4000, $date);
        if ($aResults['status']) {
            return HTMLDisplay::GetAuditAPICallsHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
     ...
     * @return type
     */
    public function GetAuditDocumentsAPICallsHTMLList() {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $date = date("Y-m-d H:i:s", strtotime('-4 weeks', time()));
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType("Document", "document", 4000, $date);
        if ($aResults['status']) {
            return HTMLDisplay::GetAuditAPICallsHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
     ...
     * @return type
     */
    public function GetAuditXMLAPICallsHTMLList() {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();
        $date = date("Y-m-d H:i:s", strtotime('-4 weeks', time()));
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByType("API", "API", 4000, $date);
        if ($aResults['status']) {
            return HTMLDisplay::GetAuditAPICallsHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
    ...
    public function GetSystemAPICallsStatus($api, $api2, $system) {
        ...
        $aResults = $aAuditsAPICallsDAO->listAuditAPICallsByTypeAndSystem(strtolower($api), strtolower($api2), $system, 1, $date);
        if ($aResults['status']) {
            return TRUE;
        }
        return FALSE;
     ...
     * @return type
     */
    public function GetTranslationConfigList($language) {
        $aTranslationConfigDAO = new TranslationConfigDAO();
        $aResults = $aTranslationConfigDAO->getTranslationConfigs($language);
        if ($aResults['status']) {
     ...
     * @return type
     */
    public function GetTranslationTrainingList($archive) {
        $aTranslationConfigDAO = new TranslationConfigDAO();
        $aResults = $aTranslationConfigDAO->listTranslationTraining($archive);
        if ($aResults['status']) {
            return HTMLDisplay::GetTranslationTrainingHTMLList($aResults['resultsArray']);
        }
        return $aResults['message'];
    }
    ...
    public static function audit($type, $text, $return, $language, $version) {
        $aAuditsAPICallsDAO = new AuditsAPICallsDAO();

        $aData[item] = $text . " (" . $language . ")";
        $aData[translation] = $return;
        $aData[type] = $type;
        $aData[caller] = $version;
