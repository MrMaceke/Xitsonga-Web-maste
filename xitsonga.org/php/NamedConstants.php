<?php
    ...
    class NamedConstants{
       ...
       public static $LIST_ENTITIES_BY_TYPE = "SELECT E.*,U.firstname, U.lastname,U.email,U.facebook_reg, U.facebook_id, I.description from entity as E inner join users as U on U.user_id = E.user_id inner join item_type as I on I.item_type = E.item_type where E.active ='1' and (LOWER(I.description) = LOWER(?1?) or LOWER(I.description) = LOWER(?4?) or LOWER(I.description) = LOWER(?5?) or LOWER(I.description) = LOWER(?6?) or LOWER(I.description) = LOWER(?7?) or LOWER(I.description) = LOWER(?8?))  order by E.entity_name asc LIMIT ?2?, ?3?";
       ...
       public static $REMOVE_ITEM_TYPE = "UPDATE item_type set active = '0' where item_type = ?1?";
        ...
        public static $LIST_ALL_AUDIT_API_CALLS_WITH_TYPE = "SELECT * from audit_api_calls where date_created >= ('?4?') AND (type like LOWER(?1?) or type like LOWER(?2?)) ORDER BY date_created asc LIMIT ?3?";
